...
def index():
    ...
    movie_from_db = current_app.db.movie.find({"_id": {"$in": user.movies}})
    movies_data = [Movie(**movie) for movie in movie_from_db]

    return render_template(
        "index.html", title="Movies Watchlist", movies_data=movies_data
    )
